...
export PROJECT_NAME=coldchaindemo
...
export TELEMETRY_TOPIC=${PROJECT_NAME}-telemetry
export REEFER_TOPIC=${PROJECT_NAME}-reefers
...
oc create configmap agent-cm \
  --from-literal=REEFER_TOPIC=${REEFER_TOPIC} \
  --from-literal=TELEMETRY_TOPIC=${TELEMETRY_TOPIC} \
