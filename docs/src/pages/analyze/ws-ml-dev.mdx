...
To define a new column to be used as label, we use the `Operation` tab to add `Conditional replace` to apply some basic logic on the sensor columns by using two thresholds: test the value of the CO2 to be greater than or equal to threshold (22):

![Define condition](images/a_anomally_def_1.png)


We add the new label (Issue =1, NoIssue=0) in the Anomaly Flag column:
