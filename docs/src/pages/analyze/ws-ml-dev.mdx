...
Please go to the OpenScale Dashboard, there you would see Add to Dashboard/ Add button that can be used to add a specific deployment for monitoring purpose. This will bring up a popup window showing list of available deployments. This list must show the deployment(s) you have created in previous steps.
...
Configure the Model info and Evalualtion details. Now it's ready for Model monitoring in UAT and you also can evalutae model performance from Openscale.
