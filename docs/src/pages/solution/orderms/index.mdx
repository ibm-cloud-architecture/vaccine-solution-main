...
The orders are persisted in and external DB2 instance running on Cloud Pack for Application. You need to get the username and password to connect to the DB2 instance and the jdbc URL (something like jdbc:db2://dashdb-....:50001/BLUDB)
...
You can define a `setenv.sh` script to export the environment variables.
...
appsody run --docker-options "-e KAFKA_BROKERS=$KAFKA_BROKERS -e KAFKA_APIKEY=$KAFKA_APIKEY -e DBPWD=$DBPWD -e DBUSER=$DBUSER -e SSLJDBCURL=$SSLJDBCURL  -v $(pwd)/certs/es-cert.jks:/deployments/certs/es-cert.jks " vaccine-order-mgr:0.0.1`
```
